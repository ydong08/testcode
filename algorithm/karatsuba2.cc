...
void karat(int* a, int* b, int* ret, int d);
...
int main() {
	int a[MAX_DIGITS];//first multipliand
	int b[MAX_DIGITS];//second multipliand
	...
	start=clock() - start;
	printNum(r,2*d);
	printf("%f ms (%d trials)\n",1000*(double)start/CLOCK_PER_SEC/i,i);
	start = clock();
	stop = start + CLOCK_PER_SEC;
	for(i=0;clock()<stop;i++)
	{
